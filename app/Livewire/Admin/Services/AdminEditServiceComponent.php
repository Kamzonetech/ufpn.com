...
class AdminEditServiceComponent extends Component
{
    ...
    public $message = [
        ...
        'photo.required' => "Please upload service photo",
    ...
    public function mount($id){
        $this->selService = $selService = Service::find($id);
        $this->title = $selService->title;
        $this->description = $selService->description;
    }
    ...
    public function updateService($formData)
    {
        ...
        if($this->photo){
            ...
            unlink('admin/assets/images/services/'.$this->selService->photo);
            if ($this->isImage($fileExtension)) {
            ...
            $this->selService->update(['photo'=>$serviceImageName ]);
        }

        $this->selService->update([
            'title' => $this->title,
            'description' => $this->description,
        ]);

        return redirect()->route('service.index')->with('feedback','Service info updated successfully');
    }

    public function render()
    {
        return view('livewire.admin.services.admin-edit-service-component')->layout('livewire.admin.layouts.app');
    }
}
